...
工作台的 AI 能力靠本机 `claude` CLI 执行,而 CLI 会把它读到的文件内容发往模型服务商。
如果你所在的公司对「客户信息上传公网 AI」有约束,这一层是给你的:

- **录入即脱敏** —— 表单填真名,落盘那一刻换成可读假名(`示例银行 → sl银行`、`张三丰 → 张sf`)。
  磁盘上存的就是假名,因此 AI 读到的天然也是假名;你打给 AI 的每一句话同样先过一遍替换
- **真名只在一处** —— `.mask/map.json`,600 权限、自动备份、已 gitignore,并在 AI 权限里被硬性拒绝读取
- **完全对照映射表** —— 纯字典替换,没有正则猜测或实体识别。**表里没有的一个字都不动**;
  加谁、删谁、补哪些简称,全由你在「映射表」页面决定
- **AI 可读范围逐目录指定** —— 知识库每个子目录两个独立开关:能不能读、提取时要不要脱敏。
  未放通的目录连同其提取文本被 `permissions.deny` 拒绝(Read / Grep / Glob 一并受限,
  **Bash 命令里的路径也受同一套规则约束** —— 实测开着 Bash 时 `ls ~/Desktop` 一样被拒)
- **Bash 默认禁用**,原因不是"防止绕过路径规则",而是它能做路径规则管不到的事:
  用 python 库直读 docx/xlsx **原件**里的真名(那些原件就在允许读的项目目录里,内容改不掉)、
  以及联网外发。需要生成 xlsx/pptx 或让 AI 给产出截图自检时可临时开启,**跑完自动关闭**
- **磁盘范围默认只有工作台** —— 家目录下除工作台自身外的所有目录一律拒绝读取,
  以后新增的目录自动挡住;要放开哪个在 AI 任务页「磁盘范围」里逐项勾选,
  也可追加外挂硬盘等任意路径。**这条是实测出来的**:改之前一个 AI 任务能列出家目录 580+ 个文件
- 我方同事、我方产品**不登记因而不脱敏**;联系方式类字段直接不存
